...
function FilePreview({ filename }) {
  ...
  return (
    <div className="rounded-2xl overflow-hidden border border-slate-800 bg-slate-950/80">
      {isPdf ? (
        <iframe src={url} className="w-full h-96" title="preview" />
      ) : (
        <img src={url} alt="preview" className="w-full object-contain max-h-96" />
      )}
    </div>
  );
}
...
export default function DocumentDetailsPage() {
  ...
  useEffect(() => {
    ...
    fetchDocumentById(id, token).then(setDocument).catch(console.error);
  }, [id]);
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex gap-3">
          ...
          <Badge status={document.validation?.status || document.status}>
            {document.validation?.status || document.status}
          </Badge>
        </div>
      </div>
      ...
      <div className="grid gap-6 xl:grid-cols-[1.2fr,0.8fr]">
        <Card title="Prévisualisation / OCR">
          <div className="space-y-4">
            <FilePreview filename={storedFilename} />

            <a
              className="inline-flex rounded-2xl border border-slate-700 px-4 py-2 text-sm text-slate-200 hover:border-slate-500 transition-colors"
              href={minioUrl}
              target="_blank"
              rel="noreferrer"
            >
        ...
        <Card title="Champs extraits">
          <div className="space-y-3">
            ...
            {!document.metadata.extractedData && (
              <p className="text-slate-400">Aucune donnée extraite pour l'instant.</p>
            )}
          </div>
        </Card>
      </div>
      ...
      <div className="grid gap-6 xl:grid-cols-2">
        <Card title="Validation et anomalies">
          <div className="space-y-3">
            {document.metadata.validation?.checks?.map((check) => (
              <div
                key={check.name}
            ...
            {document.metadata.validation?.anomalies?.length ? (
              <div className="rounded-2xl border border-amber-500/30 bg-amber-500/10 p-4 text-amber-200">
                ...
                <ul className="mt-2 list-disc pl-5 text-sm">
                  {document.metadata.validation.anomalies.map((anomaly) => (
        ...
        <Card title="Actions métier">
          <div className="space-y-4 text-sm text-slate-300">
            ...
            {document.metadata.supplierId ? (
              <div className="flex flex-wrap gap-3">
                <Link
                  className="rounded-2xl bg-brand-500 px-5 py-3 font-medium text-white"
                  to={`/crm?selected=${document.metadata.supplierId}`}
                >
                  Ouvrir dans le CRM
                </Link>
                <Link
                  className="rounded-2xl border border-slate-700 px-5 py-3 font-medium text-slate-200"
                  to={`/user?supplierId=${document.metadata.supplierId}`}
                >
